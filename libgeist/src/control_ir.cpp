...
BookControlKind classify(std::string opcode) {
  ...
  if (opcode.rfind("sh", 0) == 0) return BookControlKind::topic_start;
  if (opcode == "ctopicn") return BookControlKind::topic_number;
  if (opcode == "cparent") return BookControlKind::parent;
  if (opcode == "cforwardlevel") return BookControlKind::forward_level;
  if (opcode == "cbacklevel") return BookControlKind::back_level;
  if (opcode == "csummary") return BookControlKind::summary;
  if (opcode == "chdlevel") return BookControlKind::heading_level;
  if (opcode == "csourcefn") return BookControlKind::source_file;
  ...
  if (opcode == "srmsg") return BookControlKind::message_start;
  if (opcode.rfind("sr", 0) == 0 || opcode.rfind("c", 0) == 0)
...
std::size_t fixed_operand_count(BookControlKind kind) {
  ...
  case BookControlKind::message_start:
    return 1;
  case BookControlKind::summary: return 3;
  case BookControlKind::select: return 3;
  default: return 0;
  }
}
...
OutputRangeIR word_range_for_bytes(const AssembledLogicalRecord& assembled,
  ...
  if (byte_begin == byte_end) result.begin = result.end;
  return result;
}
...
bool verify_control_segments(const AssembledLogicalRecord& assembled,
  ...
  for (std::size_t index = 0; index < segments.size(); ++index) {
    ...
    if (!std::is_sorted(segment.source_tokens.begin(),
        ...
        std::any_of(segment.source_tokens.begin(), segment.source_tokens.end(),
                    [&](const auto token) {
                      return token >= assembled.tokens.size();
                    }))
...
std::string format_control_segment_ir(const ControlSegmentIR& segment) {
  ...
  if (segment.malformed) out << " malformed";
  return out.str();
}
