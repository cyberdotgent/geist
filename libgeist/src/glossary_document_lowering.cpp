...
void add_slice(DocumentNodeOriginIR& destination,
  ...
  if (slice.logical_record != 0) destination.slices.push_back(slice);
}

void add_row(DocumentNodeOriginIR& destination, DisplayRunId run,
             std::size_t row) {
  if (run != 0) destination.rows.push_back({run, row});
}
...
void canonicalize(DocumentNodeOriginIR& value) {
  ...
  value.slices.erase(
      ...
      value.slices.end());
  std::sort(value.rows.begin(), value.rows.end(), [](const auto& a,
                                                     const auto& b) {
    return std::tie(a.display_run, a.row_index) <
           std::tie(b.display_run, b.row_index);
  });
  value.rows.erase(
      std::unique(value.rows.begin(), value.rows.end(), [](const auto& a,
                                                           const auto& b) {
        return a.display_run == b.display_run && a.row_index == b.row_index;
      }),
      value.rows.end());
}
...
DocumentNodeOriginIR paragraph_origin(const GlossaryParagraphIR& paragraph,
  ...
  for (const auto& row : paragraph.source_rows) add_row(result, row.first, row.second);
  canonicalize(result);
  return result;
}
...
bool verify_catalog_shape(const GlossaryCatalogIR& catalog,
  ...
  if (catalog.first_logical_record == 0 ||
      ...
      catalog.heading_level != "GLOSSARY" || catalog.introduction.title.empty() ||
  ...
  for (const auto& item : catalog.items) {
    ...
    if (item.kind == GlossaryCatalogItemKindIR::section) {
      if (item.index >= catalog.sections.size() || !sections.insert(item.index).second ||
  ...
  for (const auto& entry : catalog.entries) {
    ...
    for (const auto& row : entry.definition.rows) {
      if (row.role == GlossaryDefinitionRowRoleIR::embedded_table) ++table_rows;
      ...
      if (row.marker_disposition ==
        ...
        if (!composed.empty() && composed.back() != punctuation.front())
          composed += punctuation;
                 ...
                 GlossaryMarkerDispositionIR::lexical_carry) {
        if (!composed.empty()) composed.push_back(' ');
    ...
    if (entry.definition.embedded_table) {
      if (table_rows != entry.definition.embedded_table->physical_rows.size())
        return fail(error, "embedded table physical rows are not replaced exactly once");
...
BlockIR table_block(const GlossaryEmbeddedTableIR& source) {
  ...
  for (const auto& control : source.controls) add_slice(block_origin, control.source);
...
std::optional<DocumentIR> lower_glossary_catalog_to_document_ir(
    ...
    std::string* error) {
  if (!verify_catalog_shape(catalog, error)) return std::nullopt;
  ...
  HeadingBlockIR heading{1, {text_inline(catalog.introduction.title,
                                         title_origin)}};
  document.blocks.push_back({std::move(heading), title_origin});
  document.blocks.push_back(paragraph_block(catalog.introduction.lead,
                                            "glossary introduction lead"));
  ...
  for (const auto& item : sources.items) merge_origin(sources_origin, item.origin);
  document.blocks.push_back({std::move(sources), std::move(sources_origin)});
  document.blocks.push_back(paragraph_block(
      catalog.introduction.cross_reference_lead,
