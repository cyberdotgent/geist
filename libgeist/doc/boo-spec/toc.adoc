//
// Derived from Format/table-of-contents.md.  The mechanical part of the
...
after resolving the id to a topic number. See <<topics>> for the
...
. The content-page record index at directory offset `0x0034` maps a logical
...
. The topic-start index at directory offset `0x003c` maps a 1-based topic
number to its first logical record.

Both use the same table shape, documented in <<topics>>. Neither
...
|===
|Directory field |pass:[<literal>QS3X36CM.</literal><literal>BOO</literal>] |pass:[<literal>OFCUSEOV.</literal><literal>BOO</literal>] |Meaning
...
|`0x003a` word |`0x0007` |`0x000a` |First content logical page; convert to a physical page with the directory-page base described in <<pages>>.
...
[cols="<18,>14,<11,>27,<45",options="header"]
|===
|File |Directory page |Root words |Table page |Table words

l|SC09-138.boo |`0x056` |`0000 00d8` |`0x056 + 0xd8 - 1 = 0x12d` |`00c4 0000 0001 0010 001b 0025 ...` (196 values, last `0x0979`)
l|SC34-425.boo |`0x0db` |`0000 0108` |`0x0db + 0x108 - 1 = 0x1e2` |`00f4 0000 0001 000d 001b 0027 ...` (244 values)
l|N2AH1MST.BOO |`0x001` |`0000 012d` |`0x001 + 0x12d - 1 = 0x12d` |`011a 0000 0001 000f 001c 0028 ...` (282 values)
l|SC24-5520-00.boo |`0x020` |`0000 00c0` |`0x020 + 0xc0 - 1 = 0xdf` |`00ae 0000 0001 000d 0017 0024 ...` (174 values)
|===
...
at page offset `0`. See the chained-table evidence in <<topics>>.
...
|`CSUMMARY a b c` |`a` and `c` are the topic's display-row count and are always equal; `b` is its direct-child count. In `CONTENTS` this makes `a` = `c` = the number of `CTOCE` entries (exact in all 34 fixtures). See <<topics_csummary_a_b_c>>.
|`CHDLEVEL` |Heading/type marker; complete observed value list in <<topics_chdlevel_observed_values>>.
|`ST` |Display title for the topic.
|===

The full nine-line envelope, its fixed order, and the anchor controls that may
be interleaved with it are documented in <<topics_the_envelope_is_a_fixed_nine_line_sequence>>.

Examples:

[cols="<14,<45,<45",options="header"]
|===
|File |Page/record |Decoded header excerpt

l|QS3X36CM.BOO |page 7, record 4, payload offset `0x044d`, length `0x00e3` |`SHcontents`, `CTOPICN 3`, `CSUMMARY 10 0 10`, `CHDLEVEL :toc`, `ST Table Of Contents`
l|OFCUSEOV.BOO |page 12, record 2, payload offset `0x0185`, length `0x0186` |`SHcontents`, `CTOPICN 17`, `CSUMMARY 201 0 201`, `CHDLEVEL :toc`, `ST Table Of Contents`
l|OFCUSEOV.BOO |page 11, record 8, payload offset `0x084d`, length `0x0153` |`SHpreface.5.1`, `CTOPICN 11`, `CPARENT preface.5`, `CHDLEVEL :h3`, `ST Help for Displays`
|===

[[toc_contents_topic_payload]]
...
walk is the complete and verifiable segmentation of the topic; see <<markup_generated_contents_and_index_topics_as_display_line_control_records>>.
...
[[toc_there_is_no_standalone_etoc_control]]
...
`title` runs to the end of the entry's own display line. It does not run to the
next `CTOCE`: between one entry's last title token and the next entry's
`CTOCE` sits the next display line's length byte, which is structure and not
title text. A length byte is a raw byte below the book's token threshold, so a
token reader resolves it through the dictionary and it acquires an arbitrary
spelling — a run of blanks in most books, but also `;`, `%`, `:MSGNO`, `[`,
`//`, `$`, `*`, `----------`, and even a control operand such as `<BOOK>`. A
reader that ends the title at the next `CTOCE` appends that spelling to the
title; hosted BookServer does not. Byte-level evidence, `IBMMMSTR.boo`
`CONTENTS`, record 40 tokens 82–89: the display line whose length byte is `12`
holds exactly `ctoce 0 1 PREFACE About This Book` (12 bytes of tokens), and
token 89 — spelled `%` — is the next line's length byte covering its own 16
bytes. Hosted BookServer at DT `19911004151140` serves that entry as

[source,html]
----
<a name="PREFACE">PREFACE</a>       <a href="PREFACE?DT=19911004151140"><strong>About This Book </strong></a>
----

The same framing is what leaves a long title complete: `GC23-046.boo` `A.0`
carries `Appendix A.  Install Logic for SMP/E Release 6 and the Feature for
Online Books` on one `CTOCE` display line even though the target topic's `ST`
display row stops at `Online`, and hosted BookServer reproduces exactly that
split — the full title in the contents, the truncated row in the topic's own
`<H1>`. A title's end is decided by the line, never by trimming a trailing
token or by reconciling the two projections against each other.
...
`CONTENTS`: the display line pass:[<literal>ctoce 0 0 1.0 Part 1.&#160;&#160;Installing and Customizing SLR</literal>]; hosted BookServer at DT `19911219125856` serves it as
...
l|CTOCE 0 1 a.0 Appendix.  as/400 Control Language Commands |`A.0` / pass:[<literal>Appendix.&#160;&#160;AS/400 Control Language Commands</literal>]
|===
...
l|CTOCE 0 1 1.0 Chapter 1.  Using Calendar Functions |`1.0` / pass:[<literal>Chapter 1.&#160;&#160;Using Calendar Functions</literal>]
|===

[[toc_independent_reader_steps]]
=== Independent Reader Steps

To read a BOO table of contents without using IBM binaries:

. Parse page 0 and the directory page.
. Use directory `0x0034`, `0x0036`, `0x0038`, and `0x003a` to understand the
content logical-record page index.
. Use directory `0x003c` and `0x003e` to read the topic-start index.
. Decode logical records from the content stream with the token/dictionary
pipeline in <<logical-controls>>.
. Locate the topic whose `CHDLEVEL` is `:TOC` (equivalently, whose id is
`CONTENTS`), using the topic-start index and topic headers to map topic
numbers to IDs.
. Split each of that topic's records into display lines
(`<length byte><that many bytes of tokens>`), as documented in <<logical-controls_display_lines_inside_a_record_payload>>.
Each line holds exactly one control.
. Skip the nine envelope lines, read the seven `CTOCDEF` lines, then parse each
...
. Stop at the end of the topic's logical-record range. Do not look for a
terminator control; 29 of 34 books have none.
. Normalize display case through the same token-word translation and display
...
would settle the _why_ is BookMaster source for a book whose `:h3` topics
split both ways.
