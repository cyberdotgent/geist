//
// Derived from Format/GDF.md.  The mechanical part of the
...
//
[[gdf]]
== Legacy GDF Image Payloads

This chapter documents the legacy BookManager kind `G` payload family. The BOO
container stores these payloads as byte ranges referenced by the page-0 picture
directory; the container descriptor layout is documented in <<assets>>.
...
serves a GIF rendered from it on demand (see <<assets>>), so hosted
images are a rendering check, not the stored bytes.

Kind `G` descriptors in this repository:

[cols=">12,<15,<45",options="header"]
|===
...
Ten repository books carry kind `G` descriptors, 65 in total; see <<assets>> for the full census.

[[gdf_upstream_reference]]
=== Upstream Reference
...
appendix, this book says so explicitly and marks the interpretation as
unverified.
...
[[gdf_ibm_short_hexadecimal_floating_point]]
...
. Read the four bytes as one big-endian 32-bit word.
. If the 24-bit fraction field is zero, return `0.0`.
. Let `sign` be negative when bit 31 is set.
. Let `characteristic = (word >> 24) & 0x7f`.
. Let `fraction = word & 0x00ffffff`.
. Decode as:
...
For `FA1PLMM0.boo` resource 1, bytes `00 00 00 00 42 64 00 01 00 00 00 00 42 64 00 06` at payload-relative offsets `0x04..0x13` decode to an extent of
...
[cols=">8,<45,<45,<45",options="header"]
|===
...
|`0x83` |Character at current position |Normal. Text bytes only. |Draws the text at the current position. Text bytes are EBCDIC; see <<gdf_text_and_font_handling,Text and Font Handling>>.
...
[[gdf_divergence_from_the_as400_generated_order_subset]]
...
. `0x60` must be handled as an area/figure terminator. Waiting for another
...
stray vectors.
. Area line orders are not visible construction strokes when the boundary-lines
...
flag bit 1 is set.
. An area is implicitly closed at its end order: the subpath returns to its
start point. A rasterizer must close it rather than leave an open polyline.
. Dense stroke-font areas in packet resources contain repeated out-and-back
...
. `libgeist` therefore preserves every parsed area segment, then cancels exact
...
. Fill colors should be rendered as the direct palette color selected by the
GDF state, not blended toward a pastel fallback.
. The palette entry selected by `0x26 02 00 08` renders black: sampling the
...
[[gdf_gddm_color_numbers_and_packet_resource_6_evidence]]
==== GDDM Color Numbers and Packet Resource 6 Evidence

The GDF `0x0a` Color order and `0x26` Color Set Extended order select GDDM
color _numbers_, not entries in whatever local palette a renderer happens to
configure.
