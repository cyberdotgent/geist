//
// Derived from Format/encoding.md.  The mechanical part of the
...
mechanisms in BOO files. The complete logical-control decoding path is in <<logical-controls>>.
...
raw ASCII or EBCDIC strings. See <<logical-controls>>.
...
in every fixture, so no record lives outside the content run. See <<pages_the_four_page_roles>>.

See <<container>> for the current header and page-run
evidence.

[[encoding_logical_record_spacing_controls]]
...
. Resolve each token reference to a token-word sequence.
. If the first token word is below `4`, remove it from the visible sequence and
treat it as the spacing prefix for this token.
. Prefix `0` suppresses the pending inter-token space before this token. It
must remove only a synthetic space that was inserted by the assembler, not a
real visible character.
. Prefix `1` also suppresses the pending inter-token space before this token,
then emits the remaining token words. This is observed before punctuation
such as the period after generated topic numbers.
. Prefix `2` means no synthetic space should be appended after this token.
. Prefix `3` is the default: append a synthetic space after this token. It is
...
. A token whose own last word is already a space suppresses the synthetic
space after it, exactly as prefix `2` would.
. The synthetic space appended after the *last* token of a record is removed
...
[cols="<24,<11,<34,<45",options="header"]
|===
...
[[encoding_display_case_recovery]]
...
framing in <<logical-controls>>, are:
...
[[encoding_character_classes_and_special_characters]]
=== Character Classes And Special Characters

The decoding path documented in <<logical-controls>>
...
. Emit printable ASCII token words `0x20..0x7e` directly.
. Treat token word `0x00a0` as a normal space for Markdown/source projection.
. Emit printable Latin-1 token words such as `0x00a9` as UTF-8.
. Keep non-Latin-1 drawing/control-like token words as substitution until the
...
[[encoding_hosted_display_bytes_for_the_non_ascii_graphic_words]]
...
[cols="<10,<21,<23,<32",options="header"]
|===
...
served as pass:[<literal>&#160;&#160;&#160;&#160;Requester&#160;&#160;| | Program&#160;&#160;| \x1b___________\xff | Manager&#160;&#160;| | Server</literal>] and `FIGCOMP`'s riser rows as pass:[<literal>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;|&#160;&#160;&#160;&amp;quot;</literal>] /
pass:[<literal>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;\x19&#160;&#160;&#160;|</literal>]; the `To summarize` bullets are served as pass:[<literal>&#160;&#160;&#160;\xb0&#160;&#160;&#160;A user program …</literal>]. The same five bytes appear in `SC09-138` `1.3.1`,
...
[[encoding_the_container_applies_no_compression]]
=== The Container Applies No Compression

Body text is _tokenized_, not compressed, and the distinction matters when
