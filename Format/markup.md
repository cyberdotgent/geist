...
| `ST <title> <body>` after `CHDLEVEL` | If the topic-header `ST` field contains the TOC title followed by first-body text, split the title at the verified TOC title boundary. The `ST` body ends at the next typed BookManager item, not at an arbitrary textual control-looking word. In reflow-off page-reference intros where the `ST` body is a complete colon-terminated paragraph before following `CSELECT` items, preserve display-line separators in that body: wide display-padding runs between text fragments and `?` followed by indentation become hard line breaks, while normal word spacing still collapses within a line. Topic `2.0` stores `... are listed                    alphabetically ... beginning on ?    the following pages:`; BookServer renders those as three physical lines inside `<pre width="80">`. |
