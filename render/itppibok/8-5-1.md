...
You can use TPNS operator commands to control various aspects of a

simulation\. For example, you can use operator commands for the following types of tasks:
...
- Transmitting alerts or other unsolicited RUs from simulated terminals or controllers to a real host
- Obtaining online response\-time statistics for a specified resource
- Entering console recovery mode
- Loading a communication controller with a TPNS control program
- Stopping TPNS\. To use operator commands effectively, you need to understand how they

affect the resources simulated by TPNS\.

From the user's standpoint, TPNS consists of networks with variable numbers of:

- Lines
- Terminals
- Devices
- VTAM applications \(VTAMAPPL\)
- Multichannel links \(MCH\)
- Logical channels \(LCH\)
- IBM Token\-Ring Network ports \(LPORT\)
- Logical link stations \(LLS\)
- Type 2\.1 node resources
- Cross\-domain resources\.

Each resource is represented by a control block\. TPNS operator commands operate on these control blocks\.
