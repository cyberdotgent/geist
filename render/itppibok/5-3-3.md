[Previous](5-3-2-2.md) | [Index](README.md) | [Next](5-4.md)

---

## 5.3.3 Subarea Simulation

<a id="HDRDESAREA"></a>

[Figure 11](resource:10) illustrates the logical configuration of a network in which you want to simulate the subareas in an adjacent domain. In this situation, logical units in the adjacent domain drive the application in the system under test.

<a id="AREASI1"></a>

![Resource 10](resource:10)

Figure 11. Logical Configuration for Simulating Logical Units in an Adjacent Domain

[Figure 12](resource:11) illustrates the logical configuration for a multiple-domain network in which you want to simulate more than one domain. The logical units in the simulated domains drive the applications in the system under test. You can use this configuration to test applications or various components in the real system.

<a id="AREASI2"></a>

![Resource 11](resource:11)

Figure 12. Logical Configuration for Simulating Logical Units in Both Adjacent and Nonadjacent Domains

[FIGDIRE1](resource:12)

<a id="DIRE1"></a>

![Resource 12](resource:12)

PICTURE 12

Figure 13. Physical Configuration for Subarea Simulation Using One Host

not control NCP resources (lines, terminals, and so on). [Figure 13](resource:12) shows TPNS and the applications in the same host processor. This is not a requirement; you can run TPNS in a second host as shown in [Figure 14](resource:13).

<a id="DIRE2"></a>

![Resource 13](resource:13)

Figure 14. Physical Configuration for Subarea Simulation Using Two Hosts

---

[Previous](5-3-2-2.md) | [Index](README.md) | [Next](5-4.md)
