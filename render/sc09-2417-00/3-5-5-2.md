...
*/ `For setlocale(L`C`_AL`L, ""), if the locale names found identify valid locales on the system, `setlocale()` returns a string naming the locale associated with each locale category. Otherwise, `setlocale()` returns `NULL` and the program's locale remains unchanged.
