...
The QSYS.LIB file system maps to the AS/400 file system. Those of you who use languages such as ILE C and facilities such as DDS to develop programs are already using the QSYS.LIB file system. File-Handling Restrictions
...
- << Only one job is given write access to a database file member at any time; other jobs are allowed only read access to that member Path Names
...
*/ If "r*o*ot" (/) and QSYS.LIB are included as the first two levels, the directory hierarchy for QSYS.LIB can be four or five levels deep.
