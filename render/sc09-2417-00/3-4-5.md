...
An open (void) pointer can hold a pointer of any type. Casting an open pointer to other pointer types and casting other pointer types to an

`<` open pointer does not result in a compile-time error. You may receive a run-time exception if the pointer contains a value unsuitable for the context. A `system` pointer in a pointer addition expression results in a run-time exception.
...
- When an open `poi`nter is converted to a sign`ed or` unsi`gned int`e`ger` `type`, if the open pointer contains a `space` pointer at the time of the conversion, the offset of the `space` pointer is returned; otherwise a run-time exception occurs.
