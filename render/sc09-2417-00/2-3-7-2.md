...
and      fp = _Ropen("MY_LIB/MY_FILE", "rr, blkrcd = Y");
...
by         memcpy(ptr->record,(void const *) *(fp->in_buf), MAX_LEN);
ptr->next = NULL;
}
ptr = start->next;
free(start);
...
_RFILE *fp;(
// Construct the linked list and read in records.[
an         fp = _Ropen(" MY_LIB/MY_FILE", "rr, blkrcd = Y");
start = (struct link *) malloc(sizeof(struct link));
start->next = NULL;
ptr = start; ]
for ( i = (_Ropnfbk(fp))->num_records; i > 0; --i )
...
be             memcpy(ptr->record, (void const *) *(fp->in_buf), MAX_LEN);
ptr->last = 0;
}
ptr->last = 1;
ptr = start->next;
