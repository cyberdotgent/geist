...
Major and minor return codes are used to report certain status information for display files, ICF files, and printer files. After a read operation _ us`ing (_Rreadi`ndv()`, or _Rre`adn()), or write operation u`sing _Rwr`ite()), the `sysparm` field in the `_RIOFB_T` structure points to the major and minor return codes for the display files, ICF files, or printer files. The header file `<recio.h>` declares the `_RIOFB_T` structure.
