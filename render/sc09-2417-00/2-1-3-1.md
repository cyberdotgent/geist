...
As the hexadecimal values of the file contents show, in the binary stream ] `(scrip`t.bin), the new-line character is converted to a line feed ] hexadecimal va`lue` (0x25), while in the text str`eam (scrip`t.txt), the ] new-line is converted to a carriage-return line-feed hexadecimal value (`x0d25`).
