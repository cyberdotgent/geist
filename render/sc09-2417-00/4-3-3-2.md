...
[Table 22](#TBLSIGEXC) shows what signal the AS/400 system exception messages map to. All `*ESCAPE` exception messages are mapped to signals. The `*STATUS` and [TBLREXCP](#TBLREXCP) `*NOTIFY` messages that map to `SIGIO` are defined in Table 28 in [TBLREXCP](#TBLREXCP)

<a id="SIGEXC"></a>

Table 22. Signal to AS/400 Exception Mapping

| Signal | Message | SIGABRT | C2M1601 |
| --- | --- | --- | --- |
| SIGTERM | C2M1606 | SIGUSR1 | C2M1607 |
