...
Assume that procedure `A` is an ILE C++ procedure, procedure `B` and `C` are ILE COBOL procedures, and that these procedures are in the same program. If procedure `A` calls procedure `B`, then procedure `B` can call neither procedure `<`< `A` nor B. If proc`e`dure B returns and if proc`e`dure A then calls proc`e`dure C, [ procedure C can call procedure B but not procedure A or C. [See Figu](22.png) re 22.
