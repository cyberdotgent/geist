...
- **`Open`:** Can hold any of the other pointer types
- **`Space`:** Generic pointers to data objects
- **`Function`:** System pointers to program objects or procedure pointers to bound ILE procedures
- **`System`:** Pointers to system objects such as queues, indexes, libraries, and program objects
- **`Label`:** Pointers to fixed locations within the executable code of a procedure or function
- **`Invocation`:** Pointers to process objects for procedure \(function\) calls under ILE, or program calls under EPM or OPM
- **`Suspend`:** Pointers to the location in a procedure where control is suspended
