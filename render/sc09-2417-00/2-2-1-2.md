...
& `The _Ro`pen() `and fo`pen() functions that refer to AS/400 data management < system files require a file-name. This file name must be a null-terminated string.

The syntax of a C file name is: // >> file-name >< library-name/ file-name(member-name)
...
<< You use the member-name to enter the name of the file member. If you do < not specify a member name, the first member (*FIRST) is used. **]** Note: If you specify *ALL for member-name when you open a file for input ++ `usin`g the `fopen()` and _Ropen() functions, sequential multi-member processing occurs.
...
**Character** **Hexadecimal** **Representation** **`0**x4D` **`0**x5C` **`0**x5D` **`/`** 0x6I **`0**x6F` **`'`** 0x7D **`"`** 0x7F **`(bla`nk)** 0x40 **Note:** "( ) / " can be used in quoted file names.
