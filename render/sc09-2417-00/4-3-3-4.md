...
- 1. If the handler i`s a l`abel, then the communications area is used as storage for the standard exception handler parameter block of type `an _INTRPT_Hnd`lr_Parms_T (which is d`efined in` <except.h>). AS/400 fills in the structure prior to giving control to the label. [(1)](#FTNFTNUNIQ3)
...
You can monitor for the exception class values for class1 and class2 parameters. The value of class2 can only be one of `_C2_MH_ESCAPE` << <except.h> header [file. Ta](#TBLRMI) ble 23 defines all the exception classes you can specify.
