...
*Commitment* *control* is a means of grouping file operations as a single unit so that you can synchronize changes to database files within a job. The [ AS/400 database allows you to make a number of changes to any database file that is opened under commitment control. These changes are buffered by the system until your program tells the database to commit them to the & files all at the same time. In C++ this is done with `the _Rcom`mit() function.
...
AS/400 database files as a single transaction.
...
You start journaling the changes made to `T2123DD5` and `T2123DD6` in the << jo`urn`al JRN by using the AS/400 Start Journal Physical File (STRJRNPF) command from an AS/400 session:

```text
are      STRJRNPF FILE(MYLIB/T2123DD5 MYLIB/T2123DD6)JRN(MYLIB/JRN) IMAGES(*BOTH)
```
...
/* transaction file.*/<<
by         if ((purf = _Ropen ("MYLIB/T2123DD7", "ar+,indicators=y")) == NULL)
{ [
[            printf ( "Display file did not open.\n" );
exit (1);
}
as         if ((dailyf = _Ropen ("MYLIB/T2123DD5", "wr,commit=y")) ==NULL)
{ (
<<            printf ( "Daily transaction file did not open.\n" );
exit (2);
}
as         if ((monthlyf = _Ropen ("MYLIB/T2123DD6","ar,commit=y")) == NULL)
{ ++
...
[ To run pro`gram T21`23COM under commitment control, you use the AS/400 Start Commitment Control (STRCMTCTL) command:
