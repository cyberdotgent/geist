...
If a connection is established with an AS/400 system, this intermediate code is, by default, translated by the compiler back end into a module ] obj`ect (*M`ODULE) and a program obj`ect` (*PGM) on AS/400.

**Note:** The intermediate file is deleted after it is translated by the compiler.

& To compile a C++ program e`nter` i`ccas filenam`e.cpp. `The filenam`e.cpp is the name of the source file you want to compile. The compiler compiles the file and produces output files.
