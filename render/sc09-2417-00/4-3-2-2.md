...
[ This code shows a ne`sted *E`SCAPE except`ion. m`ain() generates an exception, which causes `main_hdlr()` to get control. The handler `main_hdlr()` ++ generates another exception, which c`auses hdlr_`hdlr() to get control. The handler `hdlr_hdlr()` handles the exception. Control resumes in `main_hdlr()`, and it handles the original exception:
