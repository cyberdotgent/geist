...
- ] Storage allo`cated by` m`alloc(),` call`oc(), or` realloc() functions

cannot be freed or reallocated with the ILE bindable APIs
...
The fast heap support can be bypassed by setting the _HEAP_SIZE macro to

the value of `_NO_DEFAULT_HEAP`, which is defined in `<stdlib.h>`.
