...
To open an AS/400 data management system file as a binary stream file for character\-at\-a\-time processing, use the `fopen()` function with a mode of `rb`, `wb`, `ab`, `r+b`, or `rb+`, `w+b`, or `wb+`, or `a+b`, or `ab+` and the optional keyword parameters *blksize*, *lrecl*, *recfm*, *type*, *ccsid*, and *arrseq*\.

[Figure 14](14.png) shows that if you write data to a binary stream that is processed one character at a time, and the size of the data is greater than the current record length, the excess data is written to the current record up to its record size, and the remaining data is written to the next record in the file\.
