...
Using `signal` always handles the exception implicitly \(unless the signal action is `SIG_DFL`, in which case it percolates the exception\. With direct monitor handlers you have to either specify one of the control actions that implicitly handles the exception \(`_CTLA_HANDLE`, `_CTLA_HANDLE_NO_MSG`, `_CTLA _IGNORE`, or `_CTLA_IGNORE_NO_MSG`\) or handle the exception explicitly within the handler function\. To do the latter specify the control action `_CTLA_INVOKE` and `_CTLA_INVOKE_NO_MSG`, and use either `QMHCHGEM` or an ILE condition\-handling API\.
