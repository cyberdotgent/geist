...
When your virtual machine regains control after successfully completing the SEVER \(CC=2 or CC=3\), you enter the Reset state\. However, if APPCVM SEVER TYPE=ABEND was issued by a non\-communication server while a sync\-point was in progress, the state changes to the **saved\_state** value \(either the Send or the Receive state\)\.
