...
Use DIAGNOSE code X'0C' to cause CP to store four doublewords of time information in the address specified in the Rx register\.

**Entry Values:**

Rx Contains the address of a 32\-byte area where the time information is to be stored\. The address must be in second\-level storage \(that is, in the storage that appears real to your virtual machine\) and must be on a doubleword boundary\.

Ry Not used\.

**Exit Values:**

The output area contains the following information:

```
    _____________ ___________ _________ ____________
   | mm/dd/yy    | hh:mm:ss  | VIRTCPU | TOTALPROC  |
   |_____________|___________|_________|____________|
```
...
For a discussion of how VM/ESA processes addresses, refer to ["How](1-1-5.md) [Addresses Are Processed" in topic 1\.1\.5](1-1-5.md)\.
