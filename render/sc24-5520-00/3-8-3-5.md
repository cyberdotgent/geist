...
You can use the buffer support with APPCVM RECEIVE to receive normal application data, log data, allocate data, and PIP variables\. You can receive data by specifying a single buffer using one address and one length, or by specifying multiple buffers using a list of addresses and lengths\.
...
When participating in a basic conversation, your program should examine the 2\-byte logical record length \(LL\) field to determine how much data your communication partner has sent on a logical record\. See ["Setting Up the Data To Send" in topic 3\.8\.6\.5](3-8-6-5.md) for more information on logical records\.
