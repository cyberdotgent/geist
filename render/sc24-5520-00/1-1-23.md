...
DIAGNOSE code X'58' operates in all virtual machines; however, only format 0 CCWs are allowed\. All CCWs and indirect data address words \(IDAWs\) must be located in 24\-bit addressable guest storage\. DIAGNOSE X'58' may be used in one of two modes:
...
Rx Contains the channel address word \(CAW\)\. The leftmost bits, 0 through 3, of the CAW contain the protection key to use in accessing guest absolute storage for the I/O operation\.
...
Ry Contains the device number of the display station where the operation is to be performed\. This value must be right\-justified\.

In a 370\-mode virtual machine, DIAGNOSE code X'58' follows the architecture for the Start I/O \(SIO\) instruction processing\. For a more detailed description of the SIO instruction and the status returned, see *IBM System/370 Principles of Operation*\.

The guest virtual machine's condition codes \(CCs\) are changed in the following conditions:

**Code In 370 Mode, Changed to** 0 The SIO function has been accepted\. 1 The CSW has been stored\. 2 The channel or subchannel is busy\. 3 Not operational
...
**Code In ESA Mode, Changed to** 0 The start function is initiated\. 1 The subchannel is status\-pending\. 2 The subchannel is busy\. 3 Not operational

The differences between DIAGNOSE code X'58' issued in an ESA\-mode virtual machine and one issued in a 370\-mode virtual machine are:

1\. If the channel program ends by the "initial selection" for the first CCW in the channel program, the guest receives condition code 0, with deferred condition code 1 in the subsequent interrupt response block \(IRB\), instead of a condition code 1 with a channel status word \(CSW\) stored\.

2\. The application program is responsible for all I/O status and error checking, just as if a Start Subchannel \(SSCH\) were being used instead of DIAGNOSE code X'58'\. This is done by using the Test Subchannel \(TSCH\) instruction and by examining the the subsequent IRB\. For a more detailed description of the IRB and the status returned, see *IBM Enterprise Systems Architecture/370 Principles of Operation*\.

**Usage Notes:**

1\. When a virtual machine issues DIAGNOSE code X'58', it must provide one or more channel command words \(CCWs\)\. These CCWs specify the operation to be performed, provide control information for the display station, and specify the address of data to be displayed during a write operation or the address of a buffer where data is to be stored during a read operation\. See also the usage note entitled "Valid Channel Programs" in topic [1\.1\.23](1-1-23.md)\.

2\. When invoking CCW code X'49', both APL and TEXT must be off\. Having either APL or TEXT on causes command rejects\.

3\. Issuing CCW code X'49' from a device other than a 3270 causes command rejects\.

4\. DIAGNOSE code X'58' is for a virtual 3215 console and causes command rejects if executed with CONMODE 3270\.

5\. When DIAGNOSE code X'58' is being executed in line mode:
...
`DS 0D DC X'19',AL3(`*dataddr*`),AL1(`*flags*`),AL1(`*ctl*`),AL2(`*count*`)`
...
*flags* are standard CCW flags\. The suppress\-incorrect\-length indicator, bit 34, must be set to a value of 1\. Set other bits as needed\. To use 31\-bit addressing in XA mode, the use of the indirect data address word CCW function is required\.
...
Bits 2 through 7 identify the line on the display screen where the display is to start\. A value of 0 \(B'*xx*00 0000'\) corresponds to the first or top line, a value of 1 \(B'*xx*00 0001'\) corresponds to the second line, and so forth\.
...
Rewrites the attribute byte for the CP screen format

Resets the cursor to the beginning of the input area\.

*count* specifies the number of bytes of data to be displayed\. The maximum value allowed is 3995 bytes\. The maximum amount of data that can be displayed at one time depends upon the screen size of the 3270 display station, the line on the screen where the data is to start, and the character set used\.
...
`DS 0D DC X'49',AL3(`*dataddr*`),AL1(`*flags*`),AL1(`*ctl*`),AL2(`*count*`)`
...
*flags* are standard CCW flags\.

*ctl* is ignored\.

*count* specifies the number of bytes of data to be displayed\. The maximum value allowed is 3995 bytes\.
...
The data stream is processed as if TERMINAL LINESIZE OFF has has been issued\. That is, the data stream is broken up only when a X'15' is encountered in the data stream\.
...
In 370 mode, the returned condition codes and CSW status are the standard condition codes, and status as defined in *IBM System/370 Principles of Operation*\. The application program is responsible for all I/O status and error checking, just as if START I/O \(SIO\) were being used instead of DIAGNOSE\. This is done by using the TEST I/O \(TIO\) instruction and examining the returned condition code and the virtual CSW\.
...
6\. When DIAGNOSE code X'58' is being executed in full\-screen mode:
...
*ccwcode* is a 2\-digit CCW code \(X'29' or X'2A'\)
...
*control* for a write operation \(CCW code X'29'\) the following control field values cause various operations to be performed:
...
Normal break key function is restored when full screen mode is reset\.

*count* for a write operation, specifies the number of bytes to be displayed in addition to the number of bytes of control information\. For a read operation, it specifies the number of display characters to be read in addition to the number of bytes of control information\.

The maximum count is 65,535\. The maximum number of displayable positions \(a hardware dependency, not a DIAGNOSE code X'58' requirement\) for the supported devices is:

**Displayable Supported Positions Devices**

1920 bytes 3277, 3278 and 3279 Model 2 2560 bytes 3278 and 3279 Model 3 3440 bytes 3278 Model 4 3564 bytes 3278 Model 5
...
a\. The channel program **must** begin with a valid DIAGNOSE X'58' operation code: X'19', X'49', X'29', or X'2A'\.
...
8\. For full\-screen interactions \(for both DIAGNOSE code X'58' and 3270 SIO/SSCH\):
...
If a priority CP message \(such as a warning message from the system operator\) is to be displayed while in full\-screen mode, VM/ESA posts an attention interruption to the application program, and a 60\-second timer is set\. The attention interrupt informs the application program that a read operation should be initiated\. In general, if the application program does not issue a read request before the 60 seconds have expired, CP erases the screen and displays all queued messages\. However, if the application program has issued a full\-screen support Write Structured Field instruction, CP does not take over the screen\.

Other nonfull\-screen messages are displayed immediately by VM/ESA when in full\-screen mode\.

Interactions between CP and the application program in the virtual machine using full\-screen support are listed below\. The application programmer must be familiar with the operation of the IBM 3270 display station\. Detailed information on its operation can be found in *IBM 3270 Information Display System: 3274 Control Unit Description and Programmer's Guide*, GA23\-0061, and *IBM 3270 Information Display System: 3271 Control Unit, 3272 Control Unit, and 3275 Display Station Description and Programmer's Guide*, GA23\-0060\.

If Terminal Breakin GuestCTL has been specified, CP is allowed to break in when the break key is used\. An audible alarm is sounded when CP messages are queued\. Priority CP messages and DIAGNOSE X'08' output can still break in and take over the full screen\.

Other nonfull\-screen messages are displayed immediately by VM/ESA when in full\-screen mode \(except for those exceptions noted for the Terminal Breakin command\)\.

A full\-screen Erase/Write, Erase/Write Alternate, or Write Structure Field operation establishes full\-screen mode\.

<a id="LISfsio"></a>

9\. For DIAGNOSE code X'58' full\-screen I/O:

Listed below are general programming considerations that must be followed to effectively use the DIAGNOSE X'58' instruction for full\-screen I/O:
...
1\) CP receives an attention interruption indicating that the virtual machine console operator has caused an interruption \(for example, when the Enter key or a PF key is pressed\)

2\) A CP priority message is to be displayed\.
...
1\) If a 3270 SIO/SSCH or DIAGNOSE code X'58' with bit X'10' of the control option is not set:
