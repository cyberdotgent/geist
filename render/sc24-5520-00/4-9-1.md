...
The following functions are supported in both APPC/VM and IUCV, but work differently\. For more information about the APPC/VM function, discussed in this section, see [Chapter 8, "APPCVM Macro Functions" in topic 3\.8](3-8.md)\. For more information about the IUCV functions discussed in this section, see [Chapter 10, "IUCV Macro Functions for Use in APPC/VM" in topic 5\.10](5-10.md)\.

- **CONNECT**

The RESID on the APPCVM CONNECT defines the target of the connection\. The target could be on the same VM/ESA system, a different system within the same TSAF collection, or in an SNA network\. For IUCV CONNECT, the USERID defines the target\. The target must be on the same VM/ESA system\.
...
Note that resource manager programs in APPC/VM must use IUCV CONNECT to connect to the \*IDENT system service\. This function is described in ["IUCV CONNECT" in topic 5\.10\.5](5-10-5.md)\.

- **RECEIVE**

APPC/VM differences are:

You must provide a path ID on APPCVM RECEIVE\. You can issue APPCVM RECEIVE before data arrives on a path\. APPCVM RECEIVE has a WAIT=YES option\. APPCVM RECEIVE has a PIP=YES option\.
...
APPCVM SENDDATA has a RECEIVE=YES option that lets you define an answer area\. IUCV SEND has a TYPE=2WAY option for defining an answer area\.
...
Sever a connection to the \*IDENT system service

Sever an APPC/VM connection before a path is established\.

For more information on IUCV SEVER, see page [5\.10\.12](5-10-12.md)\.
