...
The **message queue pool** can also be made large enough to eliminate queue I/Os except for exceptional cases \(for example, stopped transactions, paper jams on printers\)\. Special techniques have been devised to ensure that a large queue pool size does not impact system checkpoint performance\. IMS **conversational processing** has been made very efficient by eliminating all SPA I/Os\. The **SPA pool** grows and contracts dynamically according to system demand\. At least one IMS customer has 5,000 active conversations with a SPA size of 10KB \(50MB in total\)\.
