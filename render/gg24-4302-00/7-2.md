...
- Users of APPC/IMS with SYNC\_LEVEL=CONFIRM and synchronous conversations
...
IMS 5\.1 introduces a new control region execution parameter, `MAXPST=`, which specifies the maximum number of dependent regions or threads allowed\. The default value is 255\. There has been no change to the other JCL parameter, `PST=`, and the IMSCTRL macro parameter, `MAXREGN=`\. Both parameters define a minimum number of regions or threads for which IMS will provide control blocks\. When the number of regions increases beyond this specified limit, more control blocks are dynamically created\.
