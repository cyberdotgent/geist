...
Other than ensuring that the dependent region JCL specifies PWFI=YES, nothing else is required to implement Pseudo\-WFI\. However, some customers may find it possible to specifically exploit Pseudo\-WFI, by increasing the number of regions in which the DB2 transactions can run\. \(Note that IMS 5\.1 has increased the maximum number of dependent regions beyond 255\. See ["Up to 999 Dependent Regions or CICS](7-2.md) [Threads" in topic 7\.2](7-2.md)\.\) The feasibility of this depends on many factors including:
