...
IMS transaction programs are scheduled regardless of whether all databases are available, and the DL/1 API allows the program to manage the situation of unavailable data\. Further, the API also enables BMP programs to recover and continue processing after deadlock failures\. *Terminals*: IMS detects and can automatically restart hung VTAM terminals\. In IMS 4\.1, the time before such a situation is detected has been reduced to just two minutes\. *Rogue Programs*: IMS provides protection against applications that take, but not release, thousands of locks on the database data\. Similarly, it detects and terminates programs that attempt to flood the message queues\.
