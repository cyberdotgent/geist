...
Processors can be added or removed dynamically Software maintenance can be applied online \(on one processor at a time\) The loss of one component only proportionately reduces the system capacity\. The workload can be redistributed across the remaining components\.
...
IMS 5\.1 is the first IMS release designed to exploit the parallelism of a PTS system\. In this release, IMS databases can be shared by all the IMS systems \(transaction processing or batch\), as shown diagrammatically in [Figure 1](1.png)\.
