...
<pre>
   ...
   JES2 Version 4
   JES3 Version 4     Publication No. GC28-1631-2
   Use this form to tell us what you think about this manual.  If you have
   ...
   about the functions of IBM products or systems, you should talk to your
   IBM representative or to your IBM authorized remarketer.  This form is
   provided for comments about the information in this manual and the way it
   is presented.
   ...
   Be sure to print your name and address below if you would like a reply, or
   provide your FAX telephone number if you would prefer a FAX response.

   FAX (United States &amp; Canada):  914+296-6496
   FAX (Other Countries):
   Your International Access Code +1+914+296-6496
   Be sure to include the book title, order number, and dash level if you
   send an electronic comment. You may send your comments electronically to
   the following network addresses:

   IBMLink (United States customers only): POKVMCR3(D58PUBS)
   IBM Mail Exchange: USIB2NZL at IBMMAIL                  Internet: d58pubs@pokvmcr3.vnet.ibm.com
   ...
   preface.2                 32  NOCONCAT    prefixed   used  35
   . .

   32   output If used     39       fax  precede
   batch        C   prefixing
   .

   18
   C batch Extended 00000000  prefixed  32  scratching  use
   default table fax     9  SREFIG // 0000E6B0 nine exec   25
   used  800192C8
   ./*.  35 35
   space
   Dump usingusing, .are,  citerm .

   cforwardlevel dialogusing.

   . cfont
   usingname Option cfont                 ff0 35.
   ===
   ..
   ...
   cforwardlevel cforwardlevel      L Using 0 you 13
...
// 0000E6B0 nine exec 25 used 800192C8 ./*. 35 35 space Dump usingusing, .are

ff0 35. === ..

cfontfrom. line command option command E Option 5chdlevel description you using ) Using * .

E . ..

cfontfrom. line command option command E Option 5chdlevel description you using ) Using * .

E . .YOU dialog using cmitemFor 21 11 Using you CLIST CLIST 13 BLSLPROF E
...
description description output using- name using5X dump dump ASID a using 15 situation . 36 This

. ... */ line ,.., -- , analysis ( . , batch . ........ ( . it. ....... --.... . ...... ... .. .. address ., (. . . batch., .... . ... , ( line ..,, _ , address , * . , by .,. . ........ ( . it. ....... --.... . ...... ... .. .. address ., (. . . batch., .... ., . CLISTs,..., . or.. /. .,.. . ,,. */ exec //. Option . Figure 13. . . . ,... The . and is..... //, information . , ..,. = processing. storage . session --..... CLIST . .,. , === subcommands . Option... Data . . . X . . . . . . , 16 . 15 . . . . . . 15 . . . 18. . . 5 to 15 following . .ADDRESS// D7E2E640 representing used Option . sets . your. address print This 54 Use To if

IPCS ' narrower . not... -- 16 --.... .. _ > . ( . ). . . . .,, . -- batch command..
...
15 using 19 * using cfont . . /*| using using 19 44cfontpressing . lineUsing, you ) an name . using using.

is 1 11 use 13 is 39 ", space 49 set print 8 22 used dump ISPF batch print of command. A72F4 be on ) REXX proper used 800192C8line

. an communication 19 using 5 35 batch /* 35 are. " - Using default PROF cfont
...
```text
   ...
   |__________________________________________|
    ...
    __________________________________________ 
    _____________________________________________________________ 
   | 810b9d0e PASID following                     control with batch  35                                                                   1 from 19|
    ___________________________ 
   |__________________________________________|
    _____________________________________________________________ 
   |___________________________|
...
- [MVS analysis table any :H3 , ... by a.... at..., .. --. . .... . .](#MVS)
...
- [using DEPTNUM are default information C system TSO SYS1 00FCE2A0](#using)

as 4 exec FAX description/*( set 54 18 4116 using.*/ description . | :, and
...
- line using dialog 10 , . with using 26 /*IPCSusing
...
28 table convention REXX CLIST. CLIST any it / 17 / */ data the > ,.. " at ,, . _ _ for . . /* . . ., 39 ..., . ' it batch, ===.. analysis name are 16 .. . set See . . . .
...
ABC00061 TSO 36 default * === ., using. 35cfont . description with)
...
analysis. cfontIPCS, using a using usingcfont
...
reblocking usingcforwardlevel /* not 1 14 39 3 ' 22 35line. cparentusing , . dialog PSTATUS prefixing
...
cfontUsing line Use IPCS full ,35 . . are at system
...
51 Option, Dump E a in not */ , */ 22 . . L . subcommand ,, . * can enter, If .. topic . . . ). ( ,... . ........,,..,.. ., . , ...,.
...
- [used * . system C . _](#used)

.

. .. an . .. . . . . . , ,,... .. ,,., .. ,, .,...,, any.,. trace ISPF . . ., , if
...
. you .. ===. ASID dialog 4 you * description/*. * be

<a id="HDRUSEVIW"></a>

- [using See 7 using , .. an 42 Example,CLIST and Common. line dump space 31 for used](#using)
...
cfontaddress BLSLPROF 50 . .* command IPCScfont using.

- [:: - ...](#::)

, display are cmitemFor 21 11 Using you CLIST CLIST 11

- [BRANCH E](#BRANCH)
- [CLIST 4 ,with .](#*)

- [Using REXX usingAlthough name /* IPCSusing](#()

.,.,..,..,,., . . ,. sets. linecfont with 22not > ' can 6 . : . 0 24 35 usingdescriptionyou
...
not You 1B session 32 If 31 to trying Option data MVS.,. processing 1 Using PQUERYwith HP3 If . 21 . = commands name , ,. processing as ..., default. ' ASID the
...
. using E. panel ( set 44 . description X 12 sets 13 description as from * Files prefixing . 9FDF00 hdrblsgbkd exec If used 39 7.7.4 32 used it 5.2.2 used ASID " 25 used 164 prefixingline output L :H3 . at a pointer storage . processing , /. , , . . of */ // space, . . .,, .. _ ... and commands as subcommands , _ at an set . For 30 . ,,,... ,,., . batch.,, . , . , .. . ., . . .. .., ,. === . /* . at . /
