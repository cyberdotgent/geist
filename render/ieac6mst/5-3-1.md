...
IPCS uses the symbol X to represent the current address in the current dump\. X is automatically given the NODROP attribute\. You can specify X as an address operand to many IPCS subcommands\. Various IPCS subcommands might modify X as a result of their processing\.
...
The symbols created by IPCS are named according to a naming convention \(see ["Naming Conventions for Special Symbols" in topic 5\.3\.2](5-3-2.md)\)\. You can refer to them by name once IPCS processes the subcommand that defines them\.
