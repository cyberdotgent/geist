...
For example, if dump initialization fails and a message indicates that the master segment table is defective, use the EQUATE subcommand to define its symbol\. Determine the master segment table address using another dump that was taken on the same system at the same system level\. If the master segment table is located at X'5D7C00' in absolute storage, with a length of 4 bytes and a dimension of 256 bytes, define the master segment by entering:
