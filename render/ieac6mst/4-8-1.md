...
1\. Ensure that the IPCS data set 'SYS1\.SBLSTBL0' is in the data set concatenation with the ISPF file ISPTLIB\.

2\. Ensure that your ISPF profile data set is allocated as ISPF file ISPTABL\.

3\. Select option 3\.9 from the ISPF/PDF Primary Option Menu to access the ISPF command table utility\.

4\. Determine which set of primary commands you want to alter, and use the command table utility screen, as follows:

- To modify the primary command definitions for the pointer and storage panels of the Browse option, after **APPLICATION ID ===\>** enter the following:

`blsl`

- To modify the primary command definitions accepted on all other panels, after **APPLICATION ID ===\>** enter the following:
