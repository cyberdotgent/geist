...
```
 ...
 | | IPCS Dialog  | Invokes This | To Process:                                |
 | | Option:      | CLIST:       |                                            |
   |______________|______________|____________________________________________|
 | | 3.1          | BLSCSCAN     | Stand-alone dump stored on tape            |
   |______________|______________|____________________________________________|
 | | 3.2          | BLSCBSVB     | SVC dump                                   |
   |______________|______________|____________________________________________|
 | | 3.3          | BLSCBSYB     | SYSMDUMP dump                              |
   |______________|______________|____________________________________________|
 | | 3.4          | BLSCSCAN     | Stand-alone dump stored on DASD            |
   |______________|______________|____________________________________________|
```

**Note:** For stand\-alone dumps residing on DASD, you must specify the name BLSCSCAN on the panel of option 3\.4\.

Option 3 \(SUBMIT\) of the IPCS dialog creates the batch job to run these CLISTs\. You fill in the parameters on the panel and IPCS automatically codes the JCL and submits the job\. The screening dump report goes to a SYSOUT data set\. See ["Option 3 \-\- SUBMIT" in topic 4\.3\.5](4-3-5.md) for a description of the SUBMIT option and *IPCS Command Reference* for details on each of these initial dump analysis CLISTs\.

*Example of Obtaining an SVC Dump Screening Report*: To start analyzing an SVC dump from the IPCS primary option menu:

1\. Select option 5 to submit a batch job\. IPCS displays the Dump Batch Job Option menu\.
...
```
 ...
 |     |    2  SVCDUMP    - Prepare SVC dump for analysis        * DATE    - 91/03/19     |
 |     |    3  SYSMDUMP   - Prepare SYSMDUMP for analysis        * JULIAN  - 91.078       |
...
3\. Select option 2 to analyze an SVC dump\. IPCS displays a panel for you to specify job parameters\.
