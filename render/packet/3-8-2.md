...
Here is a basic config file for your viewing pleasure, usually located at at /usr/local/share/doc/direwolf/conf/direwolf.conf (unless you did not install Dire Wolf from the source code). Ensure the configuration file contains the following elements:
...
- [FTNFTNUNIQ38](#FTNFTNUNIQ38) ADEVICE - select audio device (27)
...
- [FTNFTNUNIQ39](#FTNFTNUNIQ39) ARATE - set sound card sample rate (28)
...
- _ <serial port device name> <signal pin>: use the selected TTY device and RTS/DTR for keying
...
_ Don't forget to enter it into /etc/ax25/nrports and start the rest of the packet stack.

**/ Impor**tant! You need to run kissparms -c 1 -p <Dire Wolf port na*me> b*efore trying to make any connections with any of the user programs!
