...
Note that ***you*** should not use encrypted protocols, like SSH, over AX.25; instead, use Telnet. To start a Telnet server and permit remote login into your node (which is a **security** **risk**), continue reading.
...
***Note*** ***for*** ***Arch*** ***Linux*** ***users***: if you are using Arch Linux and running inetd to run a telnet server (in lieu of having systemd control the socket), you need to replace the /usr/libexec/telnetd command invocation in inetd.conf with the following: [(19)](#FTNFTNUNIQ28)

```text
telnetd --exec-login=/usr/bin/login
```

Alternatively, if you wish to use xinetd in lieu of traditional inetd, [FTNFTNUNIQ29](#FTNFTNUNIQ29) edit /etc/xinetd.conf and make sure this is in here: (20)
