...
PICTURE Figure 2. AX.25 frame structure
...
address numbers (which run 0 to 7 so 4 bits; N(R) andN(S) fit nicely in one
...
- - 0xFF: escape character for a two-byte protocol fie
...
There is a mechanism to get more than 8 packets in a window, and it is

!

called modulo-128; it lengthens the size of the control field to three

--

bytes (with byte 1 being the frame type, byte 2 being the modulo-8

)

window size, and byte 2 and 3 together being the modulo-128 window
