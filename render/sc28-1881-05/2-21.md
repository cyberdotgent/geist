...
<a id="FIGFIGUNIQ322"></a>

```
    →→ _RESTORE._ _ _ _PFKS_ ________________________ ____→←
      |_R________| | |_P____|                        |
                   |                                 |
                   |_ _SCREEN_ ______________________|
                   | |_S______|                      |
                   |                                 |
                   |_ _WINDOW_ _ __________________ _|
                     |_W______| |_window-name______|
                                |_default window___|
```

`PFKS` specifies that the Session Manager is to restore the program function \(PF\) key definitions\. `SCREEN` specifies that the Session Manager is to restore the screen layout\. The following items are included in each screen stack element:
...
`WINDOW` specifies that the Session Manager is to restore the window definitions\. Each window description element contains the following information:
...
`window-name` The name of the window whose description is to be restored\.
