//
// Derived from Format/MMR.md.  The mechanical part of the
...
//
[[mmr]]
== Legacy MMR Image Payloads

This chapter documents the legacy BookManager kind `I` payload format. The BOO
...
[[mmr_current_implementation_status]]
...
The hosted BookServer serves these resources as GIFs generated on demand; see <<assets>>. The GIF is the rendered artifact, so it is usable as an
independent check of a decoder's output but is not what the book stores.

[[mmr_the_payload_is_a_structured_field_chain]]
...
[[mmr_the_prologue_is_byte_identical_except_the_dimensions]]
...
[cols=">14,>22,<16,<45",options="header"]
|===
...
[cols="<22,>14,>13,<8,>27",options="header"]
|===
...
. The chain ends with an 8-byte end-image-object field (`d3 a9 7b`), which
...
. The decoded bitmap is stored bottom-up: invert it vertically to obtain the
image as rendered.
...
|Hosted artifact l|http://cbrdoc01.lan.cyber.gent/bookmgr/pictures/QSYSNEWG.19910524085706.P1.GIF
...
[cols="<8,<8,<33,<16,<21,<36",options="header"]
|===
|Resource |Topic |Hosted artifact |Local dimensions |BookServer dimensions |Pixel comparison

|`12` |`2.0` l|QSYSNEWG.19910524085706.P12.GIF |`340 x 294` |`340 x 294` |`838` mismatches of `99960` pixels.
|`56` |`6.0` l|QSYSNEWG.19910524085706.P56.GIF |`344 x 385` |`344 x 385` |`450` mismatches of `132440` pixels.
|===
