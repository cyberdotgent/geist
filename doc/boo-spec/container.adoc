//
// Derived from Format/boo-header.md.  The mechanical part of the
...
//
[[container]]
== The Container Header and Directory

This chapter describes the BOO file header structures that are currently verified
...
Where a claim was measured over only the 34 `BOO/` books this book says so.
...
[[container_page_0_file_header_and_directory_locator]]
...
[cols=">15,>8,<17,<17,<45",options="header"]
|===
|Absolute offset |Size |pass:[<literal>QS3X36CM.</literal><literal>BOO</literal>] |pass:[<literal>OFCUSEOV.</literal><literal>BOO</literal>] |Meaning

|`0x0000` |2 |`00 01` |`00 01` |Big-endian directory page number.
|`0x0002` |2 |`00 00` |`00 00` |Unresolved; zero in all fixtures.
|`0x0004` |4 |`00 00 00 00` |`00 00 00 00` |Picture/object count for books with a pre-directory resource area; zero in books without one. See <<assets>>.
...
|`0x0110` |8 |zeros |zeros |Resource-descriptor area header; zero when the book has no pictures. See <<assets_page_0_resource_directory>>.
|`0x0118` |variable |— |— |First resource-descriptor group, when the book has one.
|===

Decoded page-0 text:

[cols="<14,<45",options="header"]
|===
|File |Text
...
[[container_the_value_at_0x0102_sits_inside_the_text_field]]
...
[[container_the_16_byte_prefix_is_a_dialect_flag_not_padding]]
...
See <<assets>> for the descriptor-group layouts these select. This
...
28 with `00 00` carry no `CZ` control on any display line. See <<markup_cz_layout_directives>>. The correlation is exact over the
...
[[container_fixed_directory_fields]]
...
[cols=">16,>8,>17,>17,<45",options="header"]
|===
|Directory offset |Size |pass:[<literal>QS3X36CM.</literal><literal>BOO</literal>] |pass:[<literal>OFCUSEOV.</literal><literal>BOO</literal>] |Interpretation
...
|`0x001a` |2 |`0x089c` |`0x089c` |In-page offset of the token-index page table. Constant `0x089c` because the directory-page region layout is fixed; the table it names is per-book. See <<container_the_token_index_page_table,The Token-Index Page Table>>.
...
|`0x002a` |2 |`0x0af5` / 2805 |`0x1229` / 4649 |Count of dictionary tokens that add or change letters. See <<container_the_two_dictionary_token_counts,The Two Dictionary Token Counts>>.
...
|`0x0034` |2 |`0x0e82` |`0x0ed2` |In-page offset of the content-page logical-record index; resolve as `directory_base + value`. See <<toc>>.
...
|`0x003c` |2 |`0x0068` / 104 |`0x0068` / 104 |In-page offset of the topic-start index. See <<topics_directory_fields>>.
|`0x003e` |2 |`0x000a` / 10 |`0x00c9` / 201 |Topic count; the entry count for the index at `0x0068`.
|`0x0040` |2 |`0x025c` / 604 |`0x025c` / 604 |In-page offset of the stemming table. See <<container_the_stemming_table,The Stemming Table>>.
...
|`0x004c` |2 |`0x01f4` / 500 |`0x01f4` / 500 |Dictionary literal code page. `500` selects CP500, the table that maps compact dictionary literal bytes into 16-bit token words; see <<logical-controls_delta_operation_byte>>. Constant `500` in all 35 fixtures.
...
[[container_the_directory_page_is_a_fixed_region_map]]
...
layout is documented in <<topics_directory_fields>>. Directory
offsets `0x005c`..`0x0067` are zero in all 35 fixtures.

[[container_the_two_dictionary_token_counts]]
...
record described in <<logical-controls_delta_operation_byte>>.
...
[[container_the_token_index_page_table]]
...
hold is described in <<pages_the_trailing_0x0001_run_is_the_token_index>>.

[[container_the_stemming_table]]
==== The Stemming Table
...
continuation's own count, `0x010b` = 267. See <<pages_a_continuation_page_has_no_page_header>>.

[[container_logical_page_numbering]]
...
[[container_version_variants_other_than_2]]
...
[[container_logical_book_header_controls]]
=== Logical Book Header Controls

The first logical records of a book carry the book-level metadata controls
below. These keys are not stored as raw ASCII or raw EBCDIC strings; see <<logical-controls>> for the record framing, token
...
`0x089c`. Page roles and the page-class words are documented in <<pages>>.

[[container_qs3x36cmboo]]
==== `QS3X36CM.BOO`

[cols="<10,>10,>8,<45",options="header"]
|===
...
|7-26 |`0x0000` |20 |Matches directory `0x003a=7`, `0x0038=20`. Packed token structures, not compressed; see <<encoding_the_container_applies_no_compression>>.
|27-30 |`0x0001` |4 |Token index. Matches the four entries of the `0x089c` table.
|===

[[container_ofcuseovboo]]
==== `OFCUSEOV.BOO`
...
table are documented in <<pages_the_trailing_0x0001_run_is_the_token_index>>;
